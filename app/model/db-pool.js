class DBPool {
  constructor(DB) {
    ...
    this._info = [DBPool.status.FREE, DBPool.status.FREE];
  }
  ...
  get db() {
    const runningIdx = this._info.indexOf(DBPool.status.RUNNING);
    if (runningIdx !== -1) return this._pool[runningIdx];

    // return the loading db but only at first initialisation
    // const loadingIdx = this._info.indexOf(DBPool.status.LOADING);
    // if (loadingIdx !== -1) return this._pool[loadingIdx];
    return null;
  }

  getNextDb(cb) {
    const freeIdx = this._info.indexOf(DBPool.status.FREE);
    ...
    this._info[freeIdx] = DBPool.status.LOADING;
  ...
  swapDb(cb) {
    ...
    const runningIdx = this._info.indexOf(DBPool.status.RUNNING);
    this._info[loadingIdx] = DBPool.status.RUNNING;
